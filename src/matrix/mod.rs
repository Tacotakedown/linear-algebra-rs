...
impl Matrix {
    ...
    pub fn from_file(path: &str) -> Self {
        ...
        for (count, r) in content.lines().enumerate() {
    ...
    pub fn from_string(input: &str) -> Self {
        ...
        for (count, r) in rows.iter().enumerate() {
            let entries: Vec<&str> = r.split_whitespace().collect();
            let c = entries.len();
            if count > 0 && cols != c {
                panic!("Columns don't match");
    ...
    pub fn copy(&self) -> Self {
        let n_data = self.data.par_iter().cloned().collect();
    ...
    pub fn apply(&mut self, f: impl Fn(f64) -> f64 + Sync) {
        let new_data: Vec<f64> = self.data.par_iter().map(|elem| f(*elem)).collect();
        self.data = new_data;
    }

    pub fn combine<F>(&self, b: Self, f: F) -> Self
    where
        F: Fn(f64, f64) -> f64 + Sync,
    {
        ...
        new_matrix
            .data
            .par_iter_mut()
            .zip(self.data.par_iter().zip(b.data.par_iter()))
            .for_each(|(result, (a, b))| {
                *result = f(*a, *b);
            });
    ...
    pub fn dot(&self, b: Self) -> Self {
        ...
        dp.data
            ...
            .enumerate()
            .for_each(|(i, chunk)| {
                for (j, elem) in chunk.iter_mut().enumerate().take(b.cols) {
                    let mut sum = 0.0;
                    for k in 0..b.rows {
                        sum += self[i][k] * b[k][j];
                    }
